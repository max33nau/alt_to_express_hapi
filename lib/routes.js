  ...
  { method: 'POST',
    ...
    handler: function(request,reply) {  // Posts data to my mongodb
      ...
      personForDatabase.save(function(error,user){
        if(!error) {
          console.log(request.payload.username + 'was added to the database');
          reply(request.payload.username + 'was added to the database');
          return reply.view('sport');
        } else {
          console.log(error);
        }
          ...
          if(error) {
            ...
            users.save(function(error,user){
              if(error) {
                ...
                return reply(user.username +' new favorite sport:  ' +user.sport+' was saved.');
              }
            });
          }
        });
